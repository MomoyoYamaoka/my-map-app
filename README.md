...
### マップは出るが色が付かない（別媒体からアクセス時）

**原因の例:**

1. **別ドメイン・別サーバから開いている**  
   フロントが別ホスト（例: 別サイトのリンクや iframe、別デプロイ）で開かれていると、`/api/streets` がそのホストに向かい、API サーバに届かずデータが取れません。

   **対処:**  
   - 同じオリジンで配信する（フロントも API も同じ Render の URL で提供する）  
   - または、別ホストで配信する場合は API のベース URL を指定する:
     - ビルド時に `VITE_API_BASE_URL=https://あなたのAPIのURL` を設定して再ビルド  
     - または埋め込み用に、アプリ読み込み前に  
       `<script>window.__API_BASE_URL__='https://あなたのAPIのURL';</script>` を実行する

2. **API が空 or エラー**  
   バックエンドや Overpass API の不調で `/api/streets` が空配列や 5xx を返している可能性があります。

   **確認:** ブラウザの開発者ツールの **Network** で `streets` を選び、ステータスとレスポンスを確認。本番の **Render のログ** で Overpass のエラーやタイムアウトが出ていないか確認する。
